...
if (isset($_POST['add_student'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO grade_9B (student_name, birthday, age, parent_contact, lrn) VALUES (?, ?, ?, ?, ?)");
...
if (isset($_POST['edit_student'])) {
    ...
    $stmt = $conn->prepare("UPDATE grade_9B SET student_name=?, birthday=?, age=?, parent_contact=?, lrn=? WHERE id=?");
...
if (isset($_POST['delete_student'])) {
    $stmt = $conn->prepare("DELETE FROM grade_9B WHERE id=?");
...
$students = $conn->query("SELECT * FROM grade_9B ORDER BY student_name ASC");
?>
...
<h1 class="text-4xl font-bold text-center mb-6">Grade 9 - Section B</h1>
