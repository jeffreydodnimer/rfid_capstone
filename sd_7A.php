...
if (isset($_POST['add_student'])) {
    ...
    if (!isValidName($name)) {
        die("Invalid name format. Use: Lastname, Firstname Middlename");
    }

    if (!isValidLRN($lrn)) {
        die("LRN must be exactly 10 digits.");
    }

    $stmt = $conn->prepare("INSERT INTO grade_7A (student_name, birthday, age, parent_contact, lrn) VALUES (?, ?, ?, ?, ?)");
...
if (isset($_POST['edit_student'])) {
    ...
    $stmt = $conn->prepare("UPDATE grade_7A SET student_name=?, birthday=?, age=?, parent_contact=?, lrn=? WHERE id=?");
...
if (isset($_POST['delete_student'])) {
    $stmt = $conn->prepare("DELETE FROM grade_7A WHERE id=?");
    $stmt->bind_param("i", $_POST['student_id']);
    $stmt->execute();

    // No resequencing of id in DB for data safety

    header("Location: " . $_SERVER['PHP_SELF']);
    exit();
}

$students = $conn->query("SELECT * FROM grade_7A ORDER BY student_name ASC");
?>
...
<h1 class="text-4xl font-bold text-center mb-6">Grade 7 - Section A</h1>
...
<div class="max-w-6xl mx-auto bg-white p-4 rounded shadow">
    ...
    <table class="w-full table-auto border border-gray-300 text-sm text-center">
        ...
        <tbody>
            ...
            <tr>
                <td class="border px-3 py-2"><?= $counter++ ?></td> <!-- sequential number -->
