...
if (isset($_POST['add_student'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO grade_10B (student_name, birthday, age, parent_contact, lrn) VALUES (?, ?, ?, ?, ?)");
...
if (isset($_POST['edit_student'])) {
    ...
    $stmt = $conn->prepare("UPDATE grade_10B SET student_name=?, birthday=?, age=?, parent_contact=?, lrn=? WHERE id=?");
...
if (isset($_POST['delete_student'])) {
    $stmt = $conn->prepare("DELETE FROM grade_10B WHERE id=?");
...
$students = $conn->query("SELECT * FROM grade_10B ORDER BY student_name ASC");
?>
...
<head>
    <meta charset="UTF-8" />
    <title>Grade 10 - Section B</title>
...
<h1 class="text-4xl font-bold text-center mb-6">Grade 10 - Section B</h1>

<div class="max-w-6xl mx-auto bg-white p-4 rounded shadow">
    <div class="flex justify-between items-center mb-4 px-2">
        <a href="admin_dashboard.phpp" class="bg-blue-600 text-white px-4 py-2 rounded inline-flex items-center gap-2">
            ← Back
        </a>
