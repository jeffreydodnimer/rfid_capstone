...
if (isset($_POST['add_student'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO strand_tvl (student_name, birthday, age, parent_contact, lrn) VALUES (?, ?, ?, ?, ?)");
    if (!$stmt) {
        die("Prepare failed (add): (" . $conn->errno . ") " . $conn->error);
    }
...
if (isset($_POST['edit_student'])) {
    ...
    $stmt = $conn->prepare("UPDATE strand_tvl SET student_name=?, birthday=?, age=?, parent_contact=?, lrn=? WHERE id=?");
    if (!$stmt) {
        die("Prepare failed (edit): (" . $conn->errno . ") " . $conn->error);
    }
...
if (isset($_POST['delete_student'])) {
    $stmt = $conn->prepare("DELETE FROM strand_tvl WHERE id=?");
    if (!$stmt) {
...
$students = $conn->query("SELECT * FROM strand_tvl ORDER BY student_name ASC");
if (!$students) {
...
<head>
    <meta charset="UTF-8" />
    <title>Strand TVL Students</title>
...
<h1 class="text-4xl font-bold text-center mb-6">Strand TVL Students</h1>
