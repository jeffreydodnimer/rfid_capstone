...
if (isset($_POST['add_student'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO strand_humms (student_name, birthday, age, parent_contact, lrn) VALUES (?, ?, ?, ?, ?)");
...
if (isset($_POST['edit_student'])) {
    ...
    $stmt = $conn->prepare("UPDATE strand_humms SET student_name=?, birthday=?, age=?, parent_contact=?, lrn=? WHERE id=?");
...
if (isset($_POST['delete_student'])) {
    $stmt = $conn->prepare("DELETE FROM strand_humms WHERE id=?");
...
$students = $conn->query("SELECT * FROM strand_humms ORDER BY student_name ASC");
?>
...
<head>
    <meta charset="UTF-8" />
    <title>Strand HUMMS Students</title>
...
<h1 class="text-4xl font-bold text-center mb-6">Strand HUMMS Students</h1>
