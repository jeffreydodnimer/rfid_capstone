...
if (isset($_POST['add_student'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO strand_gas (student_name, birthday, age, parent_contact, lrn) VALUES (?, ?, ?, ?, ?)");
...
if (isset($_POST['edit_student'])) {
    ...
    $stmt = $conn->prepare("UPDATE strand_gas SET student_name=?, birthday=?, age=?, parent_contact=?, lrn=? WHERE id=?");
...
if (isset($_POST['delete_student'])) {
    $stmt = $conn->prepare("DELETE FROM strand_gas WHERE id=?");
...
$students = $conn->query("SELECT * FROM strand_gas ORDER BY student_name ASC");
?>
...
<head>
    <meta charset="UTF-8" />
    <title>Strand GAS Students</title>
...
<h1 class="text-4xl font-bold text-center mb-6">Strand GAS Students</h1>
