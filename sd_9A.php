...
if (isset($_POST['add_student'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO grade_9A (student_name, birthday, age, parent_contact, lrn) VALUES (?, ?, ?, ?, ?)");
...
if (isset($_POST['edit_student'])) {
    ...
    $stmt = $conn->prepare("UPDATE grade_9A SET student_name=?, birthday=?, age=?, parent_contact=?, lrn=? WHERE id=?");
...
if (isset($_POST['delete_student'])) {
    $stmt = $conn->prepare("DELETE FROM grade_9A WHERE id=?");
...
$students = $conn->query("SELECT * FROM grade_9A ORDER BY student_name ASC");
?>
...
<h1 class="text-4xl font-bold text-center mb-6">Grade 9 - Section A</h1>
